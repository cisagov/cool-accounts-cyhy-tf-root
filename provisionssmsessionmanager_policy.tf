...
# provision the SSM Document resource and set up SSM session logging in this
# account.
# ------------------------------------------------------------------------------
...
data "aws_iam_policy_document" "provisionssmsessionmanager_policy_doc" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:ssm:${var.aws_region}:${local.cyhy_account_id}:document/SSM-SessionManagerRunShell",
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:logs:${var.aws_region}:${local.cyhy_account_id}:log-group:${module.session_manager.ssm_session_log_group.name}:*",
    ]
  }
}
